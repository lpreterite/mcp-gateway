...
- **类型转换 Panic** (`src/pool/pool.go`): 修复 `Pool.CallTool` 中错误的类型断言，该错误会导致调用 MCP 工具时进程崩溃。将 `MCPClientConnection.CallTool` 的返回类型更改为 `*ToolCallResult`。
- **Node.js 环境路径** (环境): 清理了残留的空目录，确保 `mcp-gateway` 能自动检测到正确的 Node.js 路径。
- **端口冲突** (环境): 解决了由于残留进程占用 `4298` 端口导致的启动失败。
